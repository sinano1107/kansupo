...
async def handle_response(res: Response):
    ...
    global page
    global wait_for_dock_to_open_task
    ...
    if res.url.endswith("/api_port/port"):
        ...
        if ndock[0].get("api_state") == 0:
            ...
            # 入居させる処理をキューに追加
            await queue.put(start_repair)
        else:
            print("入渠ドックが埋まっているので待機します")
            print("この処理はコメントアウトしています")
            # complete_time = ndock[0].get("api_complete_time")
            # wait_seconds = complete_time / 1000 - time()
            # async def wait_and_reload():
            #     print("{}秒待ちます".format(wait_seconds))
            #     # TODO handle_in_ndock_pageに処理を入れる形で実装する。そのためにはif分岐の順序などを見直す必要がある
            ...
            # wait_for_dock_to_open_task = asyncio.create_task(wait_and_reload())
        return
